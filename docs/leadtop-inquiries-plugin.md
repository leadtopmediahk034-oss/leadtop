...
- 通过“询盘设置”启用新询盘邮件通知，并配置多个收件地址；默认包含 WordPress 管理员邮箱和 `leadtopmediahk@gmail.com`，升级时保留原收件人并自动追加这两个地址。
- 客户填写有效邮箱时，通知邮件会设置对应的 `Reply-To`；发送结果会记录在询盘元数据中，邮件失败不影响询盘保存。
- 邮件通过 WordPress 后台任务异步发送，SMTP 响应较慢时不会阻塞官网询盘提交。
- 通知邮件会包含落地页、引荐页面及 UTM 来源参数；后台询盘详情和 CSV 导出也会保留这些字段。
- 可在询盘列表勾选历史记录，通过批量操作“重新发送邮件通知”按当前收件邮箱设置重新投递。
