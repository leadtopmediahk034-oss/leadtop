...
function normalizePost(post, includeContent = false) {
  ...
  return {
    ...
    categories: categories.map(({ id, name, slug }) => ({ id, name: decodeEntities(name), slug: decodeSlug(slug) })),
...
export async function getCategories() {
  try {
    ...
    return categories.map(({ id, name, slug, count }) => ({ id, name: decodeEntities(name), slug: decodeSlug(slug), count }));
  } catch (error) {
